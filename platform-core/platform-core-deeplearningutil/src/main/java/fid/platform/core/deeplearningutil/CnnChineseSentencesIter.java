 ...
 */
@AllArgsConstructor
public class CnnChineseSentencesIter implements DataSetIterator {
    /**
	 * 
	 */
    ...
    private CnnChineseSentencesIter(Builder builder) {
    ...
    public DataSet next(int num) {
        ...
        for (int i = 0; i < num && sentenceProvider.hasNext(); i++) {
            ...
            List<String> tokens = tokenizeSentence(p.getKey());

            maxLength = Math.max(maxLength, tokens.size());
            tokenizedSentences.add(new Pair<>(tokens, p.getValue()));
        }
        ...
        for (int i = 0; i < tokenizedSentences.size(); i++) {
            String labelStr = tokenizedSentences.get(i).getValue();
        ...
        for (int i = 0; i < currMinibatchSize; i++) {
            List<String> currSentence = tokenizedSentences.get(i).getKey();
        ...
        if (minLength != maxLength) {
            ...
            for (int i = 0; i < currMinibatchSize; i++) {
                int sentenceLength = tokenizedSentences.get(i).getKey().size();
