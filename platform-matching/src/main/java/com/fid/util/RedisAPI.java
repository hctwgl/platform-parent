...
public class RedisAPI {
	...
	public static JedisPool getPool() {
		if (pool == null) {
			...
			// pool = new JedisPool(config, "127.0.0.1", 6379,5000, "fid123",
			// 0);
			String value = ReadPropertyUtil.getPropertyByName(ReadPropertyUtil.Config, "fid.matchtags.env");
			String addr = null;
			String password = null;
			if ("product".equals(value)) {
				addr = ReadPropertyUtil.getPropertyByName(ReadPropertyUtil.Config, "reids_master_addr");
				password = ReadPropertyUtil.getPropertyByName(ReadPropertyUtil.Config, "reids_master_password");
				if (addr == null) {
					pool = new JedisPool(config, "r-wz9d91d96d7230e4.redis.rds.aliyuncs.com", 6379, 50000, "Fid123456",
							0);
				} else if (password == null) {
					pool = new JedisPool(config, addr, 6379);
				} else {
					pool = new JedisPool(config, addr, 6379, 50000, password, 0);
				}
			} else {
				addr = "127.0.0.1";
				pool = new JedisPool(config, addr, 6379, 5000, "Fid123456", 7);
			}
			// pool = new JedisPool(config, addr, 6379);
			// pool = new JedisPool(config, "127.0.0.1", 6379);
			// pool = new JedisPool(config, "120.24.239.162", 6379, 5000,
			// "fid123", 0);
		}
		return pool;
	}
	...
	public static void main(String[] args) {
		...
		// List<RbsTagsWords> list = RedisEntityUtil.getObjectByColumn(new
		// RbsTagsWords(), "wordsId", strs);
		// List<String> idsList =
		// RedisAPI.getMapValues(RedisEntityUtil.getMapKey(RbsTagsWords.class.getSimpleName(),"wordsId"),
		// strs);
		...
		// System.out.println(i+"===="+RedisAPI.get("11"));
		// System.gc();
		// }

		// Long sLong = 99L;
		// RedisAPI.setString("MAX_MESSAGE_ID", sLong+"");
		Long result = Long.parseLong(RedisAPI.getString("MAX_MESSAGE_ID"));
		System.out.println(result);
		String mapKey = "henry:";
		RedisAPI.setMapValue(mapKey + "tangst:2012:something is wrong", "hello,", "thug life!");
